...
final class Admin {
	...
	public function configuration_notice() {
		...
		if ( ! empty( $payments['stripe_enabled'] ) && ( ! Settings::stripe_secret_key() || ! Settings::stripe_webhook_secret() ) ) {
			$this->warning( __( 'Membexa Stripe is enabled, but a secret key or webhook signing secret is missing.', 'membexa' ) );
		}
		if ( ! empty( $payments['paypal_enabled'] ) && ( ! Settings::paypal_client_id() || ! Settings::paypal_client_secret() ) ) {
			$this->warning( __( 'Membexa PayPal is enabled, but the Client ID or Client Secret is missing.', 'membexa' ) );
		} elseif ( ! empty( $payments['paypal_enabled'] ) && ! Settings::paypal_webhook_id() ) {
			$this->warning( __( 'Membexa PayPal is enabled without a Webhook ID. Recurring subscription lifecycle updates require a verified PayPal webhook.', 'membexa' ) );
		}
		if ( ! empty( $payments['bkash_enabled'] ) && ! Bkash::enabled() ) {
			$this->warning( __( 'Membexa bKash is enabled, but one or more merchant API credentials are missing.', 'membexa' ) );
		}
	}
	...
	public function dashboard() {
		...
		<div class="wrap membexa-admin">
			...
			<div class="membexa-stat-grid">
				<?php $this->stat( $active, __( 'Active subscriptions', 'membexa' ) ); ?>
				<?php $this->stat( $total_members, __( 'Members', 'membexa' ) ); ?>
				<?php $this->stat( $past_due, __( 'Past due', 'membexa' ) ); ?>
				<?php $this->stat( $payments, __( 'Recorded payments', 'membexa' ) ); ?>
			</div>
	...
	public function subscriptions() {
		...
		<div class="wrap membexa-admin">
			...
			<?php $this->pagination( $page, $total, $per_page ); ?>
		</div>
		<?php
	}
	 ...
	 * @return void
	 */
	private function pagination( $page, $total, $per_page ) {
	...
	private function general_settings() {
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td><input id="membexa-currency" class="small-text" maxlength="3" name="membexa_general[default_currency]" value="<?php echo esc_attr( $settings['default_currency'] ); ?>"><p class="description"><?php esc_html_e( 'Three-letter ISO currency code.', 'membexa' ); ?></p></td>
			</tr>
			<tr>
				<th><label for="membexa-pricing-page"><?php esc_html_e( 'Pricing page', 'membexa' ); ?></label></th>
				<td>
					<?php
					// Core generates the complete select element and escapes its attributes and options internally.
	...
	private function payment_settings() {
		...
		$this->paypal_settings( $settings );
		$this->bkash_settings( $settings );
	}
	 ...
	 * @return void
	 */
	private function stripe_settings( $settings ) {
		?>
		...
		<table class="form-table" role="presentation">
			...
			<tr><th><label for="membexa-stripe-webhook"><?php esc_html_e( 'Webhook signing secret', 'membexa' ); ?></label></th><td><input id="membexa-stripe-webhook" class="regular-text code" type="password" autocomplete="new-password" name="membexa_payments[stripe_webhook_secret]" value="<?php echo esc_attr( $settings['stripe_webhook_secret'] ); ?>"><?php /* translators: %s: Stripe webhook endpoint URL. */ ?><p class="description"><?php echo esc_html( sprintf( __( 'Webhook endpoint: %s', 'membexa' ), rest_url( 'membexa/v1/stripe/webhook' ) ) ); ?></p></td></tr>
		</table>
		<?php
	}
	...
	private function email_settings() {
		...
		?>
		<table class="form-table" role="presentation">
			<tr><th><label for="membexa-from-name"><?php esc_html_e( 'From name', 'membexa' ); ?></label></th><td><input id="membexa-from-name" class="regular-text" name="membexa_emails[from_name]" value="<?php echo esc_attr( $settings['from_name'] ); ?>"></td></tr>
			<tr><th><label for="membexa-from-email"><?php esc_html_e( 'From email', 'membexa' ); ?></label></th><td><input id="membexa-from-email" class="regular-text" type="email" name="membexa_emails[from_email]" value="<?php echo esc_attr( $settings['from_email'] ); ?>"></td></tr>
			<tr><th><?php esc_html_e( 'Notifications', 'membexa' ); ?></th><td><label><input type="checkbox" name="membexa_emails[activation_enabled]" value="1" <?php checked( $settings['activation_enabled'] ); ?>> <?php esc_html_e( 'Membership activated', 'membexa' ); ?></label><br><label><input type="checkbox" name="membexa_emails[cancel_enabled]" value="1" <?php checked( $settings['cancel_enabled'] ); ?>> <?php esc_html_e( 'Membership canceled', 'membexa' ); ?></label></td></tr>
		</table>
		<?php
	}

	/** Render privacy and uninstall settings. */
	private function data_settings() {
		$settings = Settings::data();
		settings_fields( 'membexa_data_group' );
		?>
		<table class="form-table" role="presentation">
			<tr><th><?php esc_html_e( 'Uninstall cleanup', 'membexa' ); ?></th><td><label><input type="checkbox" name="membexa_data[delete_on_uninstall]" value="1" <?php checked( $settings['delete_on_uninstall'] ); ?>> <?php esc_html_e( 'Permanently delete Membexa plans, access rules, options, subscriptions, and transactions when the plugin is uninstalled.', 'membexa' ); ?></label><p class="description"><strong><?php esc_html_e( 'This cannot be undone.', 'membexa' ); ?></strong></p></td></tr>
		</table>
		<?php
	}
